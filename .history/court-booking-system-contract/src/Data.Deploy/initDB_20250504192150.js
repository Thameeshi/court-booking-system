...
export class DBInitializer {
    ...
    static async init() {
        ...
        const bookingList = await this.#runSelectQuery(`SELECT COUNT(*) as count FROM ${Tables.BOOKING}s`);
        if (bookingList[0].count === 0) {
            ...
            if (courts.length > 0) {
                ...
                await this.#runQuery(`
                    INSERT INTO ${Tables.BOOKING} (UserEmail, CourtId, Date, StartTime, EndTime, Status)
                    VALUES ${insertValues}
                `);
            }
        }
