...
export class BookingService {
    ...
    async cancelBooking(courtId, reason) {
    return new Promise((resolve, reject) => {
        const updateQuery = `
        UPDATE bookings
        SET Status = 'Cancelled', CancellationReason = ?
        WHERE CourtId = ? AND Status = 'Active'
        ORDER BY Date DESC LIMIT 1
        `;

        this.#db.run(updateQuery, [reason, courtId], function (err) {
        if (err) {
            console.error("Error cancelling booking:", err.message);
            reject(err);
        } else if (this.changes === 0) {
            resolve({ success: false, message: "No active booking found for this court." });
        } else {
