        ...
        navigate("/payment", { 
          state: { 
            booking: {
              ...
              StartTime: startTime,
              EndTime: endTime,
              Id: response.id || response.bookingId || Date.now() // Include booking ID if available
            },
            court: court,
            totalAmount: (() => {
              try {
                const duration = calculateTotalAmount(startTime, endTime, court.PricePerHour);
                return duration;
              } catch (err) {
                return court.PricePerHour || 50; // Fallback amount
              }
            })()
          } 
        });
  ...
  return (
    <div className="confirm-booking-container">
      ...
      <button
        ...
        onClick={handleProceedToPayment} // <-- Call your navigation logic here
      >
        {isProcessing ? (
          <>
            <span
      ...
      {bookedSlots.length > 0 && bookingDetails.date && (
        <div className="booked-slots mt-4">
          ...
          <small className="text-muted mt-2">
            ℹ️ Cancelled and completed slots are available for booking again.
          </small>
        </div>
      )}
    </div>
