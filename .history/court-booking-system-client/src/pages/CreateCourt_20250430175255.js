...
const CreateCourt = () => {
    ...
    const fetchCourts = useCallback(async () => {
        try {
            const response = await courtService.getCourtsByOwner(userInfo.email);
            console.log("getMyCourts response:", response);

            // Check if courts list is inside response.success and is an array
            const courtList = Array.isArray(response.success)
                ? response.success
            ...
            setCourts([]); // fallback to empty array
        }
    }, [userInfo.email]);
        ...
        <div className="container mt-5">
            ...
            <form onSubmit={handleSubmit} className="card p-4 shadow-sm" encType="multipart/form-data">
                ...
                <div className="mb-3">
                    ...
                    <input type="text" name="pricePerHour" value={formData.pricePerHour} onChange={handleInputChange} className="form-control" required />
                </div>
                <div className="mb-3">
                ...
                <div className="mb-3">
                    ...
                    <input type="text" name="type" value={formData.type} onChange={handleInputChange} className="form-control" required />
                </div>
                <div className="mb-3">
                    <label htmlFor="availability" className="form-label">Availability</label>
                    <select name="availability" value={formData.availability} onChange={handleInputChange} className="form-select" required>
                ...
                <div className="mb-3">
                    ...
                    <input type="file" id="image" name="image" className="form-control" accept="image/*" onChange={(e) => setImageFile(e.target.files[0])} required />
                </div>
                    ...
                    <tbody>
                        {courts.map((court) => (
                            <tr key={court.Id}>
                                ...
                                <td>{court.Price}</td>
                                <td>
                                    <a href={`https://test.xrplexplorer.com/en/nft/${court.NFTokenID}`} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-outline-primary">
                                        View NFT
                                    </a>
                                </td>
                                <td>
                                    <a href={`https://test.xrplexplorer.com/explorer/${court.NFTokenSellOffer}`} target="_blank" rel="noopener noreferrer" className="btn btn-sm btn-outline-primary">
                                        View Offer
                                    </a>
                                </td>
                                <td>
