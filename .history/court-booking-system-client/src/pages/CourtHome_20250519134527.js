...
import UserCourtList from "../component/UserCourtList.js";
...
const HomePage = () => {
  ...
  useEffect(() => {
    const filtered = courts.filter(
      (court) =>
        ...
        court.sport?.toLowerCase().includes(sport.toLowerCase()) &&
  ...
  const fetchCourts = async () => {
    try {
      const response = await axios.get("http://localhost:3001/api/courts");
