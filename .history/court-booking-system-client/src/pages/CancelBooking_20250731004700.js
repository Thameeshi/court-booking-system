...
const CancelBooking = () => {
  ...
  const handleCancel = async (e) => {
    ...
    try {
      // Dummy ID or refactor this to use actual booking ID from route or props
      const bookingId = "123"; // replace with actual ID
      const email = "placeholder@email.com"; // replace or remove from backend if not needed
      const response = await courtService.cancelBooking({CourtId, email, reason });
      ...
      if (response && response.success) {
        ...
        setError("Could not cancel booking. Please try again later.");
      }
    } catch (err) {
