  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      // Handle error response from backend
        ...
        console.log("Backend error received:", response.error);
        
        // Check if it's a slot conflict error
        if (response.error.toLowerCase().includes("time slot is already booked") || 
            response.error.toLowerCase().includes("already booked")) {
          console.log("Slot conflict detected from backend, showing notifications");
          setShowSlotConflict(true);
          showNotification(
            "⚠ Time Slot Unavailable",
            `${startTime} - ${endTime} is already booked for "${court.Name}". Please select a different time.`
          );
          setTimeout(() => setShowSlotConflict(false), 3000);
        }
      ...
      console.log("Catch block error:", error); // Debug log
      ...
      console.log("Final error message:", message); // Debug log
      
      // Check if it's a slot conflict error
      if (message.toLowerCase().includes("time slot is already booked") || 
    ...
    <div className="confirm-booking-container">
      ...
      {showSlotConflict && (
        <div className="alert alert-warning position-fixed top-0 start-50 translate-middle-x mt-3 shadow-lg conflict-notification" 
          ...
          <div className="d-flex align-items-center justify-content-center">
            <strong>⚠ Time Slot Already Booked!</strong>
          </div>
      ...
      <form onSubmit={handleBookingSubmit}>
        ...
        <div className="form-item">
          ...
          {showSlotConflict && (
            <div className="alert alert-warning mt-2 mb-0" style={{ 
              border: '2px solid #ffc107',
              backgroundColor: '#fff3cd',
              ...
              <div className="d-flex align-items-center">
                ...
                <strong style={{ color: '#856404' }}>Time Slot Already Booked!</strong>
              </div>
              <div style={{ marginTop: '5px', color: '#856404' }}>
                This time slot is unavailable for "{court?.Name}". Please select a different time.
              </div>
            </div>
          )}
        </div>
